...
<body>
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>课程名称</label>
										<input name="kechengmingcheng" id="kechengmingcheng" name="kechengmingcheng" class="form-control" placeholder="课程名称">
									</div>
									<div class="form-group col-md-6">
										<label>课程类型</label>
										<select id="kechengleixingSelect" name="kechengleixing" class="form-control">
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>班级</label>
									...
									<div class="form-group col-md-6">
										<label>课程文件</label>
										<div>
										<div class="upload">选择文件<input name="file" type="file" class="form-control-file" id="kechengwenjianupload"></div>
										<label id="kechengwenjianFileName"></label>
										<input name="kechengwenjian" id="kechengwenjian-input" type="hidden">
										</div>
									</div>
									<div class="form-group col-md-6">
										<label>课程详情</label>
										<textarea  name="kechengxiangqing" class="form-control" id="kechengxiangqing" rows="3"></textarea>
									</div>
									<div class="form-group col-md-6">
										<label>工号</label>
	...
	<script>
			...
			http("option/kechengleixing/kechengleixing","GET",{},(res)=>{
				if(res.code == 0){
					...
					for(var i=0;i<kechengleixingOptions.length;i++){
						...
						if(ruleForm.kechengleixing == kechengleixingOptions[i]){
							kechengleixingOption.setAttribute('selected','selected');
						}
		...
		function upload(){
			...
			$('#kechengwenjianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {							
					document.getElementById('kechengwenjian-input').setAttribute('value','file/download?fileName='+data.result.file);
					if(document.getElementById('kechengwenjianFileName') != null){
						document.getElementById('kechengwenjianFileName').innerHTML = "上传成功!";
					}		
					$("#kechengwenjianImg").attr("src",data.result.file);												
				}
			});
		}  
		//取消 
		...
		function dataBind(){
			...
			var kechengleixingOptions = document.getElementsByClassName("kechengleixingOption");
			for(var kechengleixingCount = 0; kechengleixingCount < kechengleixingOptions.length;kechengleixingCount++){
				if(kechengleixingOptions[kechengleixingCount].getAttribute('value') == ruleForm.kechengleixing){
					kechengleixingOptions[kechengleixingCount].setAttribute('selected','selected');
				}
			}
			$("#banji").val(ruleForm.banji);	
			$("#tupian-input").val(ruleForm.tupian);
			$("#kechengwenjian-input").val(ruleForm.kechengwenjian);
			$("#kechengwenjianFileName").html(ruleForm.kechengwenjian);
			$("#kechengxiangqing").val(ruleForm.kechengxiangqing);
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.kechengmingcheng != null && res.data.kechengmingcheng != ''){
						...
						$("#kechengmingcheng").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.kechengleixing != null && res.data.kechengleixing != ''){

						var kechengleixingOptions = document.getElementsByClassName("kechengleixingOption");
						for(var kechengleixingCount = 0; kechengleixingCount < kechengleixingOptions.length;kechengleixingCount++){
							if(kechengleixingOptions[kechengleixingCount].getAttribute('value') == res.data.kechengleixing){
								kechengleixingOptions[kechengleixingCount].setAttribute('selected','selected');
							}
						}
					}
					...
					if(res.data != null && res.data != '' && res.data.kechengwenjian != null && res.data.kechengwenjian != ''){

						$("#kechengwenjian-input").val(res.data.kechengwenjian);
						$('#kechengwenjianFileName').val(res.data.kechengwenjian)
					}
					if(res.data != null && res.data != '' && res.data.kechengxiangqing != null && res.data.kechengxiangqing != ''){

						$("#kechengxiangqing").val(res.data.kechengxiangqing);
					}
